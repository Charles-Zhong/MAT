...
for epoch in range(args.epochs):
    ...
    for batch in train_dataloader:
        ...
        for k in range(args.sampling_times_delta):
            ...
            loss_adv.backward()
            # SGLD采样
            delta = function.update_delta(delta, args.sampling_step_delta, 0).detach()
            delta = function.SGLD(delta.detach(), args.sampling_step_delta, delta_noise_epsion).detach()
            # 更新扰动的分布均值
